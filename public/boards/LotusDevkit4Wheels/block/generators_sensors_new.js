...
\t_mpu.begin();\n`;
  ...
  // ─── MLX90614 ───────────────────────────────────────────────────────────────
  // Uses the board-level `mlx90614` instance declared in LotusDevkit4Wheels.h.
  // The board constructor already runs a 500ms retry loop calling
  // mlx90614.begin(). We deliberately do NOT call begin() again: the bundled
  // Adafruit_MLX90614 wrapper overwrites its `_working` flag on every begin()
  // attempt, and a transient second-call failure flips it false → all reads
  // return -999.
  Blockly.JavaScript['lt_mlx_begin'] = function(block) {
    return '#SETUP\n// MLX90614 already initialised by LotusDevkit4Wheels() board constructor\n#END\n';
  };
  ...
  // ─── BMP280 ─────────────────────────────────────────────────────────────────
  // Uses the board-level `bmp280` instance declared in LotusDevkit4Wheels.h.
  // LotusDevkit4Wheels() already calls bmp280.begin() in the constructor, so
  // the begin block is a no-op comment — declaring a parallel `_bmp` instance
  // would waste RAM + diverge state from the board's `bmp280`.
  Blockly.JavaScript['lt_bmp280_begin'] = function(block) {
    return '#SETUP\n// BMP280 already initialised by LotusDevkit4Wheels() board constructor\n#END\n';
  };

module.exports = function(Blockly) {
  ...
  // Column-major LSB-first format for SSD1306Wire drawFastImage:
  // each byte = 8 vertical pixels at one column, LSB = topmost row.
  function toBitmapHexColumnMajor(data, width, height) {
    var heightBytes = Math.ceil(height / 8);
    var buf = new Uint8Array(width * heightBytes);
    for (var x = 0; x < width; x++) {
      for (var y = 0; y < height; y++) {
        if (data[(y * width + x) * 4] > 127) {
          buf[x * heightBytes + Math.floor(y / 8)] |= (1 << (y % 8));
        }
      }
    }
    var hex = '';
          ...
          .appendField(new Blockly.FieldImage(PLACEHOLDER, 128, 64, 'click to upload', function() {
            ...
            ;(async function() {
              ...
              img.onload = function() {
                ...
                block.bitmapHex_ = toBitmapHexColumnMajor(imgData.data, w, h);
                block.imgW_ = w;
                block.imgH_ = h;
  ...
  Blockly.Blocks['i2c128x64_display_draw_progress_bar'] = {
    init: function() {
      ...
      this.setNextStatement(true, null);
      this.setColour(basic_colour);
      this.setTooltip('draw round rect / progress bar on the screen');
      this.setHelpUrl('');
    }
  };
  ...
  Blockly.Blocks['led_green_on'] = {
    init: function() {
      ...
      this.setNextStatement(true, null);
      this.setColour(basic_colour);
      this.setTooltip('turn on green LED');
      this.setHelpUrl('');
    }
  };
  ...
  Blockly.Blocks['led_green_off'] = {
    init: function() {
      ...
      this.setNextStatement(true, null);
      this.setColour(basic_colour);
      this.setTooltip('turn off green LED');
      this.setHelpUrl('');
    }
  };

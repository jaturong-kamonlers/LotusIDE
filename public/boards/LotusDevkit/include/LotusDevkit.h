  ...
  for(address = 1; address < 127; address++ ) {
    ...
    if (error == 0) {
      ...
      switch(address) {
        ...
        case 0x20: Serial.print("PCF8574 Button"); break;
...
void LotusDevkit() {
  ...
  display.drawString(64, 36, "Devkit 2.0");
  display.display();
  ...
  // Restore SSD1306Wire's default alignment so user sketches that call
  // drawString(x, y, ...) get (x, y) as the top-left corner. The splash
...
void wait() {
  ...
  display.drawString(64, 22, "Start Button");
  display.drawString(64, 40, "GPIO27");
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.display();

...
void DallasTemperature::begin(void) {
	...
	//i don't know why first time search not work, so we hack a bit in here
	//=============================//
	_wire->reset_search();
...
//============ KBIDE Exported ===========//
float DallasTemperature::getTemperatureC(uint8_t index)
{
	requestTemperatures(); // Send the command to get temperatures
  	return getTempCByIndex(index);
}

float DallasTemperature::getTemperatureF(uint8_t index)
{
	requestTemperatures(); // Send the command to get temperatures
  	return getTempFByIndex(index);
}
...
float DallasTemperature::getTemperatureC(String address)
{
	if(address.length() != 16){ //8byte = 16chars
		return -999;
	}
	DeviceAddress deviceAddress;
	for(int i=0; i<8; i++){
		char f = address.charAt(i*2);
		char l = address.charAt(i*2+1);
		deviceAddress[i] = (nibble(f) << 4) || (nibble(l));
	}
	requestTemperatures();
	return getTempC(deviceAddress);
}

float DallasTemperature::getTemperatureF(String address)
{
	if(address.length() != 16){ //8byte = 16chars
		return -999;
	}
...
String DallasTemperature::getAddress(uint8_t index){
	DeviceAddress deviceAddress;
	bool res = getAddress(deviceAddress, index);
	String addr_str = "";
	if(res){
		for(uint8_t i=0; i<8; i++){ // 8 byte of addr
			addr_str += (deviceAddress[i] <= 0x0F)? "0"+String(deviceAddress[i], HEX) : String(deviceAddress[i], HEX);
		}
	}
	return addr_str;
}
